        ...
        <div class="cards-container">
          ...
          >
            <div class="work-img-wrap">
              <img class="work-img" :src="work.image" :alt="work.title" @error="onImageError">
            </div>
            <div class="work-info">
              <div class="work-title" v-html="highlightText(work.title)"></div>
              <div class="work-desc-wrap">
                <div class="work-desc" v-html="highlightText(work.description)"></div>
              </div>
              <div class="work-author-row">
                <div class="work-author">{{ work.author }}</div>
                <div class="work-time">{{ work.time }}</div>
              </div>
            </div>
          </div>
        </div>
...
import { useWorksData, registerCallbacks } from '../utils/dataLoader.js'
...
// 使用全局缓存的数据 - useWorksData() 现在返回 computed
...
// 本地作品数据 - 直接使用全局缓存的 computed
// (works 已经是从 useWorksData() 获取的 computed)
...
const filteredWorks = computed(() => {
  return works.value.filter(work => {
    // 分类筛选
    const matchCategory = currentCategory.value === 'all' || work.category.includes(currentCategory.value)
    
    // 搜索筛选
    const matchSearch = !searchKeyword.value || 
      isMatch(work.title, searchKeyword.value) || 
      isMatch(work.description, searchKeyword.value)
...
.work-card.selected {
  border-color: var(--accent);
  box-shadow: 0 8px 30px rgba(94, 129, 244, 0.2);
  transform: translateY(-8px);
}

.work-card:not(.selected):hover {
  border-color: var(--accent);
